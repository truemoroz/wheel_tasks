    ...
    <Container maxWidth="xs" sx={{ mt: 10 }}>
      <Paper elevation={3} sx={{ p: 4 }}>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <TextField label={t('passwordLabel')} type="password" value={password} onChange={(e) => setPassword(e.target.value)} required fullWidth autoComplete="current-password" />
        ...
        <Typography variant="body2" textAlign="center" mt={3}>
          {t('noAccount')}{' '}
          <Link href="/register" style={{ color: 'inherit', fontWeight: 600 }}>{t('registerLink')}</Link>
        </Typography>
      </Paper>
    </Container>
  );
